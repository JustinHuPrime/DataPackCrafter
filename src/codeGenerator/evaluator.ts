...
export class Evaluator implements ExpressionVisitor {
  ...
  evaluateExpectType(
    ...
    if (expectedType != null && typeof result !== expectedType) {
      ...
      throw new DSLTypeError(
        expression,
        `expected type ${desc} ${expectedType}, got ${typeof result}`,
      );
    }
    return result;
  }
  ...
  updateStore(
    name: string,
    value: Store.FunctionValue | Store.AdvancementValue,
    sourceExpression: Expression,
  ) {
    if (STORE.has(name)) {
      throw new DSLNameConflictError(
        sourceExpression,
        `function / advancement name collision on ${name}`,
      );
    }
    ...
    for (let fnArgName of astNode.args) {
      if (fnArgNames.has(fnArgName.id)) {
        throw new DSLSyntaxError(
          astNode,
          `define: Duplicate function argument name ${fnArgName}`,
        );
      }
  ...
  visitLet(astNode: Let, env: EvaluatorEnv): EvaluatorData {
    ...
    for (let idx in astNode.ids) {
      ...
      if (id == null) {
        throw new DSLSyntaxError(
          astNode,
          `let: Expected string ID in index ${idx}, got ${id}`,
        );
      }
      ...
      if (expr == null) {
        throw new DSLSyntaxError(
          astNode,
          `let: Expected expression in index ${idx}, got ${expr}`,
        );
      }
  ...
  visitIf(astNode: If, env: EvaluatorEnv): EvaluatorData {
    ...
    if (typeof value !== "boolean") {
      throw new DSLTypeError(
        astNode,
        `incorrect type of operand for if (expected boolean, got ${typeof value})`,
      );
    }
    if (value) {
  ...
  visitFor(astNode: For, env: EvaluatorEnv): EvaluatorData {
    ...
    if (typeof iterableValue !== "string" && !Array.isArray(iterableValue)) {
      throw new DSLTypeError(
        astNode,
        `for: expected list or string as target, got ${typeof iterableValue}`,
      );
    }
    ...
    switch (op) {
      ...
      case BinaryOperator.OR:
        if (lhsType !== "boolean" || lhsType !== rhsType) {
          throw new DSLTypeError(
            expr,
            `incorrect type of operand for AND/OR (expected boolean; got ${lhsType})`,
          );
        }
        if (lhsType !== rhsType) {
          throw new DSLTypeError(
            expr,
            `type mismatch for operands (got ${lhsType} and ${rhsType})`,
          );
        }
        return;
      ...
      case BinaryOperator.GTE:
        if (lhsType !== "string" && lhsType !== "number") {
          throw new DSLTypeError(
            expr,
            `incorrect type of operand for compare (expected string or number; got ${lhsType})`,
          );
        }
        if (lhsType !== rhsType) {
          throw new DSLTypeError(
      ...
      case BinaryOperator.ADD: {
        if (
          ...
          throw new DSLTypeError(
            expr,
            `incorrect type of operand for add (expected string, number, or list; got ${lhsType})`,
          );
        }
        if (lhsType !== rhsType) {
          throw new DSLTypeError(
      ...
      case BinaryOperator.MOD: {
        if (lhsType !== "number") {
          throw new DSLTypeError(
            expr,
            `incorrect type of operand for arithmetic (expected number, got ${lhsType})`,
          );
        }
        if (lhsType !== rhsType) {
          throw new DSLTypeError(
    ...
    switch (astNode.op) {
      case UnaryOperator.NEG:
        if (typeof value !== "number") {
          throw new DSLTypeError(
            astNode,
            `incorrect type of operand for negation (expected number, got ${typeof value})`,
          );
        }
        return -value;
      case UnaryOperator.NOT:
        if (typeof value !== "boolean") {
          throw new DSLTypeError(
            astNode,
            `incorrect type of operand for NOT (expected boolean, got ${typeof value})`,
          );
        }
        return !value;
    }
  ...
  visitIndex(astNode: Index, env: EvaluatorEnv): EvaluatorData {
    ...
    if (typeof targetValue !== "string" && !Array.isArray(targetValue)) {
      throw new DSLTypeError(
        astNode,
        `index: cannot index value of type ${typeof targetValue}`,
      );
    }
    ...
    if (typeof indexValue !== "number") {
      throw new DSLTypeError(
        astNode,
        `index: expected number as index, got ${typeof indexValue}`,
      );
    }
    let result = targetValue[indexValue];
    if (result == null) {
      throw new DSLIndexError(
        astNode,
        `index: index ${indexValue} invalid or out of range`,
      );
    }
    return result;
  }
  visitSlice(astNode: Slice, env: EvaluatorEnv): EvaluatorData {
    ...
    if (typeof targetValue !== "string" && !Array.isArray(targetValue)) {
      throw new DSLTypeError(
        astNode,
        `slice: bad type for target (expected string or list, got ${typeof toValue})`,
      );
    }
    if (typeof fromValue !== "number") {
      throw new DSLTypeError(
        astNode,
        `slice: bad type for start argument (got ${typeof fromValue})`,
      );
    }
    if (typeof toValue !== "number" && toValue !== undefined) {
      throw new DSLTypeError(
        astNode,
        `slice: bad type for end argument (got ${typeof toValue})`,
      );
    }
  ...
  visitCall(astNode: Call, env: EvaluatorEnv): EvaluatorData {
    ...
    if (!(fnClosure instanceof FunctionClosure)) {
      throw new DSLTypeError(
        astNode,
        `call: attempted to call non-function ${JSON.stringify(fnClosure)}`,
      );
    }
    if (astNode.args.length !== fnClosure.fn.args.length) {
      throw new DSLSyntaxError(
        astNode,
        `call: function ${fnName} expects ` +
    ...
    for (let idx in astNode.args) {
      ...
      if (fnArgName == null || fnArgExpr == null) {
        throw new DSLSyntaxError(
          astNode,
          `call: could not read arguments for ${fnName} (either name ` +
            `${JSON.stringify(fnArgName)} or arg ${JSON.stringify(
              fnArgExpr,
            )} are null`,
        );
      }
  ...
  visitList(astNode: List, env: EvaluatorEnv): EvaluatorData {
    return astNode.elements.map((expr) => {
  ...
  visitAdvancement(astNode: Advancement, env: EvaluatorEnv): EvaluatorData {
    ...
    if (astNode.name != null) {
      let evalName = this.evaluateExpectType(
        astNode.name,
        env,
        "string",
        "advancement name",
      ...
      if (!VALID_MC_ID_REGEX.test(evalName)) {
        throw new DSLSyntaxError(
          astNode,
          `advancement: invalid advancement name ${JSON.stringify(evalName)}`,
        );
      }
      name = evalName;
    }
    ...
    for (let element of astNode.details) {
      if (element instanceof Title) {
        ...
        iconItem = this.evaluateExpectType(
          element.icon,
          env,
          "string",
          "advancement icon",
        ...
        description = this.evaluateExpectType(
          element.description,
          env,
          "string",
        ...
        parent = this.evaluateExpectType(
          element.parent,
          env,
          "string",
          "advancement parent",
  ...
  visitFunction(astNode: MCFunction, env: EvaluatorEnv): EvaluatorData {
    ...
    if (astNode.name != null) {
      ...
      if (!VALID_MC_ID_REGEX.test(evalName)) {
        throw new DSLSyntaxError(
          astNode,
          `function: invalid name ${JSON.stringify(evalName)}`,
        );
      }
      name = evalName;
    }
